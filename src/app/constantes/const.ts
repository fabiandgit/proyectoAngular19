//home
export const welcome = 'WELCOME';

///nav
export const Task = 'Task';
export const addTask = 'Por favor Ingresa una tarea';
export const Weather = 'Weather';
export const lightMode = 'Modo Claro';
export const darkMode = 'Modo Oscuro';

//todo-list
export const pendingTasks = 'Tareas Pendientes';
export const textTodoList = 'Felicidades! no tienes tareas pendiente 👍';

//weather
export const weather = 'Clima';
export const SearchWeather = 'Realiza tu busqueda';
export const TitletableWeather: any = [
  'country',
  'weather',
  'description',
  'temp',
  'feels_like',
  'humidity',
  'pressure',
  'temp_max',
  'temp_min',
  'rain',
  'wind',
];
export const GetCity = 'La busqueda fue exitosa';
export const errorGetCity = 'No se encontro la ciudad';
export const errorGetApi = 'Problema al obtener los datos de la api';

//typeButtons
export const buttonSearch = 'Buscar';
export const buttonCreate = 'Crear';

//alerts
export const successAlert = '¡Éxito!';
export const warningAlert = '¡Alerta!';
export const errorAlert = '¡Error!';
